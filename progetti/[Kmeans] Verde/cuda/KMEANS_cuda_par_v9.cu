...
__global__ void assignmentKernel(double*  points, double*  d_centroids, int P, int K, int D, int* classMap, int* changed, int tileK, int pPad) {
    ...
    for (int d = 0; d < D; d++) {
      ...
      __syncthreads();
        ...
        for (int tk = 0; tk < actualTileK; tk++) {
          ...
          double dist = euclideanDistanceKernelNotSqrt(points, shmem, D, pPad, tileK, idx, tk);
          int globalK = tileStart + tk;
    ...
    int oldClass = classMap[idx];
    classMap[idx] = bestK + 1;

    if (oldClass != classMap[idx]) {
    atomicAdd(changed, 1);
  }
}
        ...
        for (int tk = 0; tk < kLoc; tk++) {
            ...
            for (int dStart = 0; dStart < D; dStart += tileD) {
                ...
                for (int d = 0; d < dLoc; d++) {
                    double diff = points[(dStart + d) * pPad + idx] - shmem[d * tileK + tk];
                    dist += diff * diff;
                }
                __syncthreads();
            }
...
__global__ void updateKernelSumAndCount(double*  points, int*   classMap, int P, int K, int D, double* auxCent, int* PointsPerClass, int  tileK, int pPad, size_t offsetTileK) {
  ...
  for (int t = threadIdx.x; t < offsetTileK; t += blockDim.x)
      sPointsPerClass[t] = 0;
  ...
  if (idx < P) {
    int cid = classMap[idx] - 1;
    if (cid >= tileStart && cid < tileEnd) {
      atomicAdd(&sPointsPerClass[cid], 1);
      for (int d = 0; d < D; d++){
        atomicAdd(&sAuxCent[d * tileK + (cid - tileStart)], points[d * pPad + idx]);
      }
    }
  }
    __syncthreads();
...
//kernel per fare la somma dei punti appartenenti allo stesso centroide e per contare quanti punti appartengo a quel centroide
__global__ void updateKernelSumAndCountTileD(double*  points, int*   classMap, int P, int K, int D, double* auxCent, int* PointsPerClass, int  tileK, int pPad, int tileD, size_t offsetTileK) {
  //salvo su shared in modo da poter ridurre intrablocco li
  extern __shared__ char shmem1[];
  int*   sPointsPerClass = (int*) shmem1;
  ...
  if (idx < P) {
    ...
    if (cid >= k0 && cid < kEnd) {
        int kLoc = cid - k0;
        if (blockIdx.z == 0){
        atomicAdd(&sPointsPerClass[kLoc], 1);} //solo i thread con indice z=0 aggiungono 1 al conto dei punti per classe
        for (int dAbs = d0, dLoc = 0; dAbs < dEnd; dAbs++, dLoc++) {
          atomicAdd(&sAuxCent[dLoc * tileK + kLoc], points[dAbs * pPad + idx]);
      }
     }
   }
     __syncthreads();
    ...
    for (int t = threadIdx.x; t < lK; t += blockDim.x) {
      int globalCid = k0 + t;
      if (blockIdx.z == 0){
        atomicAdd(&PointsPerClass[globalCid], sPointsPerClass[t]);} //solo i thread con indice z=0 aggiungono 1 al conto dei punti per classe
...
void makeGridBlock1D(int N, dim3* block, dim3* grid, void* kernel, int dev, cudaDeviceProp prop, const char* kernelName){
  ...
  while (tpb * regsPerThread > maxReg){
    tpb -= warp;}
  //dovesse essere per evitare divisione per 0
  if (tpb < warp) {tpb = warp;}

  tpb = MIN(tpb,512); //grazie a questa limitazione riesco a mettere 2 blocchi x SM
  ...
  double occupancy = (double)tpb * blocksPerSM / maxThrSM;

  printf("tpb: %d, nBlock: %d, maxThrSM: %d, maxBlocksPerSM: %d\n", tpb, nBlock, maxThrSM, blocksPerSM );
  printf("Occupancy: %.2f%% kernel: %s \n", occupancy * 100, kernelName);
...
int main(int argc, char* argv[])
{
  ...
  * arg[7] file salvataggio parziale tempi
  ...
  initCentroidsSoa(data, centroids, centroidPos, P, D, K, pPad);
...
if(inShared){
  ...
  makeGridBlock1D(P, &blockPtAssignment, &gridPtAssignment, (void*)assignmentKernel, dev, prop, "assignmentKernel");

  //k*d + k thread per updateKernelInit

  shmemSum = offsetIntTileKSum  * sizeof(int) + tileKSumAndCount*D*sizeof(double);
  makeGridBlock2DTileK(P, K, tileKSumAndCount, &blockSumAndCount, &gridSumAndCount, (void*)updateKernelSumAndCount, dev, prop, "sumAndCount");
  ...
  makeGridBlock1D(P, &blockPtAssignment, &gridPtAssignment, (void*)assignmentKernelTileD, dev, prop, "assignmentKernel");
  ...
  makeGridBlock3DTileKTileD(P, K, D, tileKSumAndCount, tileDSumAndCount, &blockSumAndCount, &gridSumAndCount, (void*)updateKernelSumAndCountTileD, dev, prop, "sumAndCount");
